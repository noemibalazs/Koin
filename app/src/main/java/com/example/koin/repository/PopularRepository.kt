...
class PopularRepository(service: MovieService, sharedPrefHelper: SharedPrefHelper): BaseRepository<MovieList>(service, sharedPrefHelper) {

    override fun loadMovies(): LiveData<MovieList> {
        return fetchData { service.getPopularMovies(KEY) }
    }
}